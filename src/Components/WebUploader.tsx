...
export default function URLUploader() {
    ...
    const { userData, loading, setError, processing, uploadProgress } = usePreflight({ url, checkPreflight, setCheckPreflight, setNewDataHash, sourceType: 'url' })
            ...
            {(loading || processing) ? null : <>
                <div className="w-1/2 mx-auto flex flex-col gap-3">
                    <form onSubmit={(e) => { e.preventDefault(); handleNewUrl() }} className="flex flex-col gap-3">
                        <input id="urlInput" type="text" required className="flex flex-1 border rounded-sm px-2 py-3" value={url} placeholder="Paste the URL here..." onChange={handleInputChange} onSubmit={handleNewUrl} />
